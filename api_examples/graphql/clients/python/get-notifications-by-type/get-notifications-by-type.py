...
@click.option('--notification_type', default="resource_created,resource_updated,resource_deleted", help="Set the notification type, for more information see https://turbot.com/v5/docs/concepts/notifications#notification-types")
@click.option('--datetime_filter', default="<T-10d", help="Configures the date range to filter the result, for more information see https://turbot.com/v5/docs/reference/filter#datetime-filters")
@click.option('--sort', default="-timestamp", help="The field to use for sorting the results, for more information see https://turbot.com/v5/docs/reference/filter#sorting")
def run_controls(config_file, profile, notification_type, datetime_filter, sort):
    ...
    query = '''
        query {
        notifications(filter: "type:%s timestamp:%s sort:%s") {
            items
            {
            notificationType
