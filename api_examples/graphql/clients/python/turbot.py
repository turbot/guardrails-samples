...
class Config:
    ...
    def __init__(self, custom_config_file, config_profile):
        ...
        # Option 2: Use environment variables
            ...
            config_dict[config_profile]['accessKey'] = os.getenv("TURBOT_ACCESS_KEY_ID")
            config_dict[config_profile]['secretKey'] = os.getenv("TURBOT_SECRET_ACCESS_KEY")
            ...
            # Option 2b: Use TURBOT_GRAPHQL_ENDPOINT environment variable
                ...
                if os.getenv("TURBOT_GRAPHQL_ENDPOINT").endswith(graphql_path):
                    config_dict[config_profile]['workspace'] = os.getenv("TURBOT_GRAPHQL_ENDPOINT")[:-len(graphql_path)]
                else:
        ...
        if config_dict:
            ...
            if self.workspace.endswith("/"):
                self.graphql_endpoint = "{}{}".format(self.workspace, graphql_path)
                self.health_endpoint = "{}{}".format(self.workspace, health_path)
            else:
                self.graphql_endpoint = "{}/{}".format(self.workspace, graphql_path)
                self.health_endpoint = "{}/{}".format(self.workspace, health_path)
            ...
            self.test_health()
            auth_bytes = '{}:{}'.format(self.accessKey, self.secretKey).encode("utf-8")
