...
def get_records(raw_records, region, account_id):
    ...
    for raw_record in raw_records:
        ...
        if finding_id in records:
            ...
            exist_record_dt = dt.datetime.fromisoformat(exist_record_timestamp[0:len(exist_record_timestamp)-1])
            new_record_dt = dt.datetime.fromisoformat(new_record_timestamp[0:len(exist_record_timestamp)-1])

            if exist_record_dt < new_record_dt:
                print(f"${finding_id} record update")
                print(notification)
                records[finding_id] = notification
        else:
            print(f"${finding_id} record created")
            print(notification)
            records[finding_id] = notification
