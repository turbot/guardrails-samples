...
def get_records(raw_records, region, account_id, processed_records):
    ...
    for raw_record in raw_records:
        ...
        sqs_message_details = {
            "id": raw_record["messageId"],
            "receiptHandle": raw_record["receiptHandle"]
        }

        # sqs_client = boto3.client('sqs')

        # sqs_client.delete_message_batch(
        #     QueueUrl="https://sqs.eu-west-2.amazonaws.com/210125595713/turbot-firehose-notification-queue",
        #     Entries=[
        #         {
        #             "Id": raw_record["messageId"],
        #             "ReceiptHandle": raw_record["receiptHandle"]
        ...
        if old_control["state"] == "tbd" and control["state"] == "ok":
            ...
            processed_records.append(sqs_message_details)
            continue
        ...
        if finding_id in records:
            exist_record_timestamp = records[finding_id]["notification"]["turbot"]["createTimestamp"]
            ...
            if exist_record_dt < new_record_dt:
                ...
                records[finding_id] = {"notification": notification, "sqs_message_details": sqs_message_details}
            else:
...
def batch_resolve_findings(client, findings, processed_records):
    workflow = {"Status": "RESOLVED"}
    response = client.batch_update_findings(FindingIdentifiers=findings,  Workflow=workflow)
    print(response)


def batch_reopen_findings(client, findings, processed_records):
    workflow = {"Status": "NEW"}
    response = client.batch_update_findings(FindingIdentifiers=findings,  Workflow=workflow)
    print(response)


def batch_import_findings(client, findings, processed_records):
...
def lambda_handler(event, context):
    try:
        ...
        processed_records = []
        records = get_records(raw_records, region, account_id, processed_records)

        securityhub_client = boto3.client('securityhub')
        sqs_client = boto3.client('sqs')
        # existing_findings = find_existing_findings(client, list(records.keys()))
        existing_findings = find_existing_findings_map(securityhub_client, records)

        product_arn = os.getenv("SECURITY_HUB_PRODUCT_ARN")
        sqs_queue_url = os.getenv("SQS_NOTIFICATION_QUEUE")
        ...
        for key in records:
            notification = records[key]["notification"]
            ...
            if key in existing_findings:
                ...
                if notification_dt <= existing_finding_dt:
                    ...
                    processed_records.append(records[key]["sqs_message_details"])
                    continue
                ...
                if notification["control"]["state"] in ["ok", "skipped", "tbd"]:
                    print(f"No finding in error exist: {key}")
                    processed_records.append(records[key]["sqs_message_details"])
                    continue
        ...
        if len(reopen_findings):
            batch_reopen_findings(securityhub_client, reopen_findings, processed_records)

        if len(resolved_findings):
            batch_resolve_findings(securityhub_client, resolved_findings, processed_records)

        if len(insert_findings):
            batch_import_findings(securityhub_client, insert_findings, processed_records)
