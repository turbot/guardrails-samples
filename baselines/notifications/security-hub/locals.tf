locals {
  ...
  private_subnet_id = var.enabled_caching ? local.create_vpc ? aws_subnet.private[0].id : data.aws_subnet.private[0].id : ""
  public_subnet_id  = var.enabled_caching ? local.create_vpc ? aws_subnet.public[0].id : data.aws_subnet.public[0].id : ""
