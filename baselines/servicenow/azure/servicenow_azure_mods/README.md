# ServiceNow-Azure Mods Installation

Turbot provides numerous ServiceNow-Azure mods, covering a wide range of Azure resources with thousands of policies and controls. By default, mods are installed with the top Turbot resource as the parent, meaning administrators must have Turbot/Owner permissions at the Turbot resource level to install, uninstall, or update mods in the environment.
...
To install ServiceNow-Azure mods using Terraform:
...
1. Navigate to the `servicenow_azure_mods` folder.
2. Run the command:
...
If you choose not to use an input variable file, the command will install **all** available Azure mods. To proceed with this option, run:
