...
variable "public_access_block_settings_skip_list" {
  type        = list(string)
  description = <<DESC
  The Public Access Block settings for the S3 Account.
  ...
  Turbot to use the current value of the account.
...
variable "smart_folder_title" {
  ...
  default     = "S3 Account Public Access Skip Settings"
}
...
variable "smart_folder_description" {
  ...
  type        = string
  default     = <<DESC
  The policy AWS > S3 > Account > Public Access Block > Settings will currently either set each setting to enabled or 
  disabled. 
