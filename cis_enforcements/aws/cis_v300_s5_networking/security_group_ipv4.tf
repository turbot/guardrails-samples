...
resource "turbot_policy_setting" "aws_vpc_security_group_ingress_rules_approved_ipv4" {
  ...
  note     = "AWS CIS v3.0.0 - Controls: 5.2"
}

# AWS > VPC > Security Group > Ingress Rules > Approved > Rules
resource "turbot_policy_setting" "aws_vpc_security_group_ingress_rules_approved_rules_ipv4" {
  resource = turbot_smart_folder.aws_cis_v300_s5_networking.id
