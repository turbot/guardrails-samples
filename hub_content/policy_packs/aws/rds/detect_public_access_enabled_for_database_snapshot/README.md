...
brand_color: "#FF9900"
display_name: "Detect if public access is enabled for RDS database snapshot"
short_name: "detect_public_access_enabled_for_database_snapshot"
description: "Guardrails policy pack to delete RDS db snapshots that is publicly accessible."
mod_dependencies:
  - "@turbot/aws"
  - "@turbot/aws-iam"
  - "@turbot/aws-rds"
...
# Detect if public access is enabled for RDS database snapshot

This policy-pack deletes RDS db snapshots that are publicly accessible, using Terraform. It automates the creation and setup of necessary Guardrails policies to which will allows Guardrails to automatically detect and delete publicly accessible db snapshots.
...
cd guardrails-samples/policy_packs/aws/rds/detect_public_access_enabled_for_database_snapshot
```
...
- Select the "MANAGE" link and `+ Add` the `Detect if public access is enabled for RDS database snapshot` Policy Pack from the dropdown menu.
- Select "Save".
...
Within the Guardrails UI navigate to:
  ...
  ```sh
  {workspace-url}/apollo/controls/explore?filter=controlTypeId%3A%27tmod%3A%40turbot%2Faws-rds%23%2Fcontrol%2Ftypes%2FdbSnapshotManualApproved%27
  ```
